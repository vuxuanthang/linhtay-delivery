import { Component, OnInit, OnDestroy } from '@angular/core';
import { HttpResponse, HttpErrorResponse } from '@angular/common/http';
import { Subscription } from 'rxjs/Subscription';
import { JhiEventManager, JhiAlertService } from 'ng-jhipster';

import { BlockMySuffix } from './block-my-suffix.model';
import { BlockMySuffixService } from './block-my-suffix.service';
import { Principal } from '../../shared';

@Component({
    selector: 'jhi-block-my-suffix',
    templateUrl: './block-my-suffix.component.html'
})
export class BlockMySuffixComponent implements OnInit, OnDestroy {
blocks: BlockMySuffix[];
    currentAccount: any;
    eventSubscriber: Subscription;

    constructor(
        private blockService: BlockMySuffixService,
        private jhiAlertService: JhiAlertService,
        private eventManager: JhiEventManager,
        private principal: Principal
    ) {
    }

    loadAll() {
        this.blockService.query().subscribe(
            (res: HttpResponse<BlockMySuffix[]>) => {
                this.blocks = res.body;
            },
            (res: HttpErrorResponse) => this.onError(res.message)
        );
    }
    ngOnInit() {
        this.loadAll();
        this.principal.identity().then((account) => {
            this.currentAccount = account;
        });
        this.registerChangeInBlocks();
    }

    ngOnDestroy() {
        this.eventManager.destroy(this.eventSubscriber);
    }

    trackId(index: number, item: BlockMySuffix) {
        return item.id;
    }
    registerChangeInBlocks() {
        this.eventSubscriber = this.eventManager.subscribe('blockListModification', (response) => this.loadAll());
    }

    private onError(error) {
        this.jhiAlertService.error(error.message, null, null);
    }
}
